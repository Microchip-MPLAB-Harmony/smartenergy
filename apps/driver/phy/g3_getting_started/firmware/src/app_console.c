...
static void _APP_CONSOLE_ShowMenu(bool fullMenu)
{
    switch (appConsoleData.state)
    {
        ...
        case APP_CONSOLE_STATE_TYPING:
        {
            ...
            APP_CONSOLE_Print("\r\n>>> %.*s", appConsoleData.txDataIndex - 2,
                    plcTxBuffer + 2);
            break;
        }
        ...
        uint16_t rssi, uint8_t lqi)
{
	...
	/* Extract real data length from two first bytes. It does not have to be the
     * same as dataLen because dataLen may contain padding length */
	msgLen = pData[0] << 8;
	msgLen += pData[1];

	if (msgLen > dataLen)
    {
		/* Length error: length in message content should never be more than
         * total data length from PHY */
    ...
    switch ( appConsoleData.state )
    {
        ...
        case APP_CONSOLE_STATE_CONFIG_MOD:
        {
            ...
            if (_APP_CONSOLE_CheckRcvChar(&rcvChar))
            {
                ...
                switch (rcvChar)
                {
                    ...
                    case '3':
                        ...
                        if (appPlcData.plcBand == G3_ARIB)
                        {
                             ...
                             */
                            APP_CONSOLE_Print("\r\n8PSK modulation not "
                                    "supported in ARIB band. Skipping "
                                    "configuration\r\n");
                    ...
                    case '4':
                        ...
                        if (appPlcData.plcBand == G3_ARIB)
                        {
                            /* Coherent Modulation Scheme not supported in ARIB
                             * band */
                            APP_CONSOLE_Print("\r\nCoherent modulation not "
                                    "supported in ARIB band. Skipping "
                                    "configuration\r\n");
                            validModulation = false;
                        }
                    ...
                    case '7':
                        ...
                        if (appPlcData.plcBand == G3_ARIB)
                        {
                            /* Coherent Modulation Scheme not supported in
                             * ARIB band */
                            APP_CONSOLE_Print("\r\nCoherent modulation not "
                                    "supported in ARIB band. Skipping "
                                    "configuration\r\n");
                ...
                if (validModulation)
                {
                    /* Set new Modulation Type & Modulation Scheme. Get maximum
                     * data length allowed with configured TX Parameters */
