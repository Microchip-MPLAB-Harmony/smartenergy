...
/* Static Notching Data
  ...
  Summary:
    Default Static Notching arrays for each newBand.
  ...
  Remarks:
    Only needed to define the static notching for the newBand(s) in use.
...
/* PLC Coupling configuration data
  ...
  Summary:
    Holds PLC configuration data for each newBand

  Description:
    This structure holds the PLC coupling configuration data for each newBand.
    It is only used if APP_CONFIG_PLC_COUP is true in the "user.h" file.

  Remarks:
    Only needed to define the coupling configuration data for the newBand(s) in use.
    They are defined in the "user.h" file.
...
static void _APP_PLC_DataIndCb(DRV_PLC_PHY_RECEPTION_OBJ *indObj, uintptr_t context)
{
	...
	/* The result is reported in crcOk (1: OK; 0: BAD; 0xFE: Timeout Error; 0xFF: CRC capability disabled) */
	if (indObj->crcOk == 1) {
...
static void _APP_PLC_GetPhyVersion(void)
{
    ...
    /* versionNum[2] correspons to G3 newBand [0x01: CEN-A, 0x02: FCC,
     * 0x03: ARIB, 0x04: CEN-B]. Set number of carriers, number of subbands and
     * Tone Map size depending on the newBand in use. */
    ...
    default:
        APP_CONSOLE_Print("Unknown newBand)\r\n");
        break;
    }
}
...
static void _APP_PLC_SetConfiguration(void)
{
    ...
    /* This function shows how to configure different PIB parameters on PLC
     * transceiver. The user can customize it depending on the requirements */

    /* Force Transmission to VLO mode by default in order to maximize signal
     * level in any case */
...
static void _APP_PLC_SetStaticNotching(void)
{
     ...
     * corresponding to the newBand can be notched (no energy is sent in those
     ...
     * The length is the number of carriers corresponding to the newBand in use
    ...
    if (appPlcData.toneMaskConfig)
    {
        ...
        /* Select the Tone Mask corresponding to the newBand in use */
...
#else
                APP_CONSOLE_Print("Warning: STATIC_NOTCHING_CENA not defined in user.h. ");
...
#else
                APP_CONSOLE_Print("Warning: STATIC_NOTCHING_CENB not defined in user.h. ");
                staticNotchingDefined = false;
#endif
                break;
            }
...
#else
                APP_CONSOLE_Print("Warning: STATIC_NOTCHING_FCC not defined in user.h. ");
                staticNotchingDefined = false;
#endif
                break;
            }
...
#else
                APP_CONSOLE_Print("Warning: STATIC_NOTCHING_ARIB not defined in user.h. ");
                staticNotchingDefined = false;
#endif
                break;
            }   
...
static void _APP_PLC_SetCouplingConfiguration(void)
{
     ...
     * it may be needed to calibrate and obtain your own values.
     * MCHP PHY Calibration Tool should be used. Here, all parameters related to
    ...
    if (appPlcData.couplingConfig)
    {
        ...
        /* Select the Coupling parameters corresponding to the newBand in use */
        switch (appPlcData.plcBand)
        {
            case G3_CEN_A:
            {
        ...
        if (couplingParamsDefined)
        {
            ...
            /* Adjust your own parameters : PLC_ID_PREDIST_COEF_TABLE_HI. (numCarriers * 2) bytes */
            ...
            /* Adjust your own parameters : PLC_ID_PREDIST_COEF_TABLE_VLO. (numCarriers * 2) bytes */
...
/*******************************************************************************
  ...
  Summary:
    Calculates maximun length in bytes allowed depending on configuration
  
  Description:    
    The maximum data length depends on:
      - G3 newBand (CENELEC-A, CENELEC-B, FCC or ARIB)
      ...
      - Number of Reed-Solomon blocks (rs2Blocks). Only applies to FCC newBand.
        1 or 2 blocks. With 2 blocks the maximum length is the double
      - Tone Map (toneMap). Dynamic notching: The subbands used to send the data
...
static uint16_t _APP_PLC_GetMaxPsduLen(void)
{
	...
	memcpy(maxPsduLenParamsObj.toneMap, appPlcData.plcTxObj.toneMap, appPlcData.toneMapSize);
...
static void _APP_PLC_TxParamsInit(void)
{
	...
	uint8_t toneMapByteIndex;
	uint8_t toneMapBitIndex;

	/* Configure Tone Map (Dynamic Notching) to use all carriers. Each bit
     * corresponds to a subband. If the bit is '1' the subband is used to carry
     * data. If the bit is '0' the subband does not carry data, but energy is
     * sent in those carriers. The number of subbands is different in each G3
     * newBand (CENELEC-A, CENELEC-B, FCC, ARIB). See "drv_plc_phy_comm.h". The
     * number of carriers per subband is 6 for CENELEC-A, 4 for CENELEC-B and
     * 3 for FCC and ARIB. Full Tone Map: 0x3F0000 (CENELEC-A, 6 subbands);
     * 0x0F0000 (CENELEC-B, 4 subbands); 0xFFFFFF (FCC, 24 subbands);
     * 0xFFFF03 (ARIB, 18 subbands). The next loop shows how to go across all
     * subbands from lower to higher frequency */
	memset(appPlcData.plcTxObj.toneMap, 0, TONE_MAP_SIZE_MAX);
	for (subbandIndex = 0; subbandIndex < appPlcData.numSubbands; subbandIndex++) {
		/* Byte index in Tone Map array */
		toneMapByteIndex = subbandIndex >> 3;

		/* Bit index in Tone Map byte */
		toneMapBitIndex = subbandIndex - (toneMapByteIndex << 3);

		/* Set subband active */
		appPlcData.plcTxObj.toneMap[toneMapByteIndex] |= (1 << toneMapBitIndex);
	}
...
void APP_PLC_Tasks ( void )
{
    ...
    switch ( appPlcData.state )
    {
        ...
        {
#if (APP_CONFIG_PLC_MULTIBAND == true)
            /* Select PLC Binary file for multi-newBand solution */
            ...
            {
                APP_CONSOLE_Print("\r\nOpening PLC driver: Loading PHY binary\r\n");
                appPlcData.state = APP_PLC_STATE_OPEN;
            }
            else
            {
                /* Unexpected error opening the PLC driver */
                APP_CONSOLE_Print("\r\nUnexpected error opening PLC driver\r\n");
        ...
        case APP_PLC_STATE_OPEN:
        {
            ...
            if (drvPlcStatus == SYS_STATUS_ERROR)
            {
                ...
                APP_CONSOLE_Print("\r\nCRITICAL ERROR: PHY binary load failed\r\n");
                appPlcData.state = APP_PLC_STATE_ERROR;
            }
            else if (drvPlcStatus == SYS_STATUS_READY)
            {
                ...
                /* Get PHY version to know the G3-PLC newBand */
