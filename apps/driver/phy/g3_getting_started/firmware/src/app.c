...
static bool _rxCheckCrc(DRV_PLC_PHY_RECEPTION_OBJ *indObj);
...
static void _pl360DataCfmCb(DRV_PLC_PHY_TRANSMISSION_CFM_OBJ *cfmObj, uintptr_t context )
{
    ...
    switch(cfmObj->result)
    {
        ...
        case DRV_PLC_PHY_TX_RESULT_INV_TONEMAP:
            SYS_DEBUG_Print("...DRV_PLC_PHY_TX_RESULT_INV_TONEMAP\r\n");
            break;
...
static uint8_t _getPaddingLength(DRV_PLC_PHY_MOD_TYPE modType, uint16_t dataLength)
{
...
#if (DRV_PLC_PHY_PLC_PROFILE == G3_CEN_A)
	numCarriers = NUM_CARRIERS_CENELEC_A;
	flBand = 4;
#elif (DRV_PLC_PHY_PLC_PROFILE == G3_CEN_B)
	numCarriers = NUM_CARRIERS_CENELEC_B;
	flBand = 4;
#elif (DRV_PLC_PHY_PLC_PROFILE == G3_FCC)
